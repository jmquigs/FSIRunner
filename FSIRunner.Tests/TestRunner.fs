...
let srcRoot = 
    if hasTempTestDir then "TempTestDir"
    else Path.Combine("..", "..", "TempTestDir")

let runnerScriptsRoot = 
    if hasTempTestDir then "."
    else Path.Combine("..", "..")
...
let cleanupTestDirectory destRoot = 
    if Directory.Exists destRoot then 
        ...
        let iterFn = 
            if dryRun then (fun f -> printfn "would delete %s" f)
            else 
                (fun fileOrDir -> 
                if File.Exists(fileOrDir) then File.Delete(fileOrDir)
                else Directory.Delete(fileOrDir))
        do Directory.GetFiles(destRoot, "Plugin*.fsx", SearchOption.AllDirectories) |> Seq.iter iterFn
...
let setupTestDirectory() = 
    ...
    let rec copyFilesToDest src dest = 
        ...
        let subcopied = List.fold2 (fun acc spath dpath -> (copyFilesToDest spath dpath @ acc)) [] srcdirs destdirs
