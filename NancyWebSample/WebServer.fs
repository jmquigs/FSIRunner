...
module private NancyHelpers = 
    type RouteDescriptionProvider() = 
        interface Nancy.Routing.IRouteDescriptionProvider with
            member x.GetDescription(mdl : Nancy.INancyModule, path : string) = 
                sprintf "%A in %A" path (mdl.GetType().FullName)
    ...
    type RootPathProvider() = 
        ...
        interface Nancy.IRootPathProvider with
            member x.GetRootPath() = Environment.CurrentDirectory
    ...
    type Bootstrapper(modules : System.Type list) = 
        ...
        let modules = 
            modules
            |> List.map (fun m -> new Nancy.Bootstrapper.ModuleRegistration(m))
            |> Seq.ofList
        ...
        override x.AutoRegisterIgnoredAssemblies = 
            let ignored : seq<Func<Assembly, bool>> = (Seq.ofList [ //for x in Nancy.DefaultNancyBootstrapper.DefaultAutoRegisterIgnoredAssemblies do
                                                                    //    yield x
                                                                    yield new Func<Assembly, bool>(fun asm -> true) ])
            ignored
        
        override x.Modules = modules
        override x.InternalConfiguration = ic
        override x.RootPathProvider = rpp :> IRootPathProvider

// Helper for interacting with the NancyModule's async handler format
let asyncHelper block = (fun token -> Async.StartAsTask(block, Tasks.TaskCreationOptions.None, token))
