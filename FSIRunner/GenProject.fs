    ...
    let generate inputFSProjectFile outFile (excludedFiles: string list) (referenceOrder: string list) = 
        ...
        let getReferenceFiles() =
            ...
            let rfiles = 
                ...
                // TODO: I think I can remove this, using the #I solution below is simpler and avoids another error case
        ...
        do 
            ...
            let refs = getReferenceFiles() 
            refs 
                |> Seq.map (fun r -> Directory.GetParent(r).FullName)
                |> Set.ofSeq
                |> Set.iter (fun rParent ->  
                    // generate a "#I" command for the reference's parent directory.  This avoids obscure "Could not load file or assembly"
                    // exceptions when there are assemblies that reference each other in the list.
                    sw.WriteLine("#I \"" + rParent + "\""))
            refs |> Seq.iter (fun r ->
